...
class Deliverys extends MY_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load_global();
        $this->load->model('deliverys_model', 'deliverys');
    }
    ...
    public function index()
    {
        ...
        $data = $this->data;
        $data['page_title'] = "liste de liveraisons";
        $this->load->view('deliverys-view', $data);
    }
    public function add()
    {
    ...
    public function newcustomers()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            $result = $this->deliverys->verify_and_save();
            echo $result;
        } else {
    ...
    public function update($id)
    {
        ...
        $data = $this->data;
        $result = $this->deliverys->get_details($id, $data);
    ...
    public function update_customers()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            $result = $this->deliverys->update_customers();
            echo $result;
        } else {
            echo "Please Fill Compulsory(* marked) Fields.";
        }
    ...
    {
        $list = $this->deliverys->get_datatables();

        var_dump($list);
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $deliverys) {
            ...
            // $row[] = ($customers->credit_limit == -1) ? "<span class='badge'>Aucune limite</span>" : store_number_format($customers->credit_limit);
        ...
        $output = array(
            "draw" => $_POST['draw'],
            // "recordsTotal" => $this->deliverys->count_all(),
            // "recordsFiltered" => $this->deliverys->count_filtered(),
            "data" => $data,
        );
    ...
    public function update_status()
    {
        ...
        $result = $this->deliverys->update_status($id, $status);
        return $result;
    }
    ...
    public function delete_customers()
    {
        ...
        return $this->deliverys->delete_customers_from_table($id);
    }
    public function multi_delete()
    {
        $this->permission_check_with_msg('customers_delete');
        $ids = implode(",", $_POST['checkbox']);
        return $this->deliverys->delete_customers_from_table($ids);
    }
    public function show_pay_now_modal()
    {
        $this->permission_check_with_msg('sales_payment_add');
        $customer_id = $this->input->post('customer_id');
        echo $this->deliverys->show_pay_now_modal($customer_id);
    }
    public function save_payment()
    {
        $this->permission_check_with_msg('sales_payment_add');
        echo $this->deliverys->save_payment();
    }
    public function show_pay_return_due_modal()
    {
        $this->permission_check_with_msg('sales_return_payment_add');
        $customer_id = $this->input->post('customer_id');
        echo $this->deliverys->show_pay_return_due_modal($customer_id);
    }
    public function save_return_due_payment()
    {
        $this->permission_check_with_msg('sales_payment_add');
        echo $this->deliverys->save_return_due_payment();
    }
    public function delete_opening_balance_entry()
    {
        $this->permission_check_with_msg('sales_payment_delete');
        $entry_id = $this->input->post('entry_id');
        echo $this->deliverys->delete_opening_balance_entry($entry_id);
    }
    /*27-06-2020*/
    public function view_payment_list_modal()
    {
        $this->permission_check_with_msg('sales_payment_add');
        $customer_id = $this->input->post('customer_id');
        echo $this->deliverys->view_payment_list_modal($customer_id);
    }
    /*28-06-2020*/
    ...
    {
        echo $this->deliverys->getCustomersJson($id);
    }
}
