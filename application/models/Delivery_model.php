...
class Delivery_model extends CI_Model {
	...
	private function _get_datatables_query()
	{
	    ...
	    //}
	     ...
	     $delivery_from_date = system_fromatted_date($delivery_from_date);
	     $delivery_to_date = $this->input->post('delivery_to_date');
	     $delivery_to_date = system_fromatted_date($delivery_to_date);
	     $users = $this->input->post('users');
	     /*if($users && !empty($users)){
	     	$this->db->where("upper(a.created_by)",strtoupper($users));
	     }*/
	     if($delivery_from_date!='1970-01-01'){
	     	$this->db->where("a.delivery_date>=",$delivery_from_date);
	     }
	     if($delivery_to_date!='1970-01-01'){
	     	$this->db->where("a.delivery_date<=",$delivery_to_date);
	     }
	...
	public function verify_save_and_update(){
		...
		$this->db->trans_begin();
		$delivery_date=system_fromatted_date($delivery_date);
	    ...
	    if($command=='save'){//Create delivery code unique if first time entry

				return "failed";
		}
		else if($command=='update'){	

			$q4 = $this->db->query("select * from db_delivery where sales_id=$sales_id")->row();
			...
			if(count($q4)==0){		
				...
				$delivery_entry = array(
						'delivery_code' 				=> get_init_code('delivery'),
						'count_id' 					=> get_count_id('db_delivery'), 
						'sales_id' 					=> $sales_id,
					'reference_no' 				=> $reference_no, 
						...
						'status' 					=> 1,
								...
								$delivery_entry['store_id']=(store_module() && is_admin()) ? $store_id : get_current_store_id();  	
								$delivery_entry['warehouse_id']=(warehouse_module() && warehouse_count()>1) ? $warehouse_id : get_store_warehouse_id();
								$q1 = $this->db->insert('db_delivery', $delivery_entry);
								$delivery_id = $this->db->insert_id();
	
								##############################################END
								$prev_item_ids = $this->db->select("item_id")->from("db_deliveryitems")->where("delivery_id",$q4->id)->get()->result_array();
	
	
								$q11=$this->db->query("delete from db_deliveryitems where delivery_id='$q4->id'");
				...
				$delivery_entry['store_id']=(store_module() && is_admin()) ? $store_id : get_current_store_id();  	
				$delivery_entry['warehouse_id']=(warehouse_module() && warehouse_count()>1) ? $warehouse_id : get_store_warehouse_id();
				$q1 = $this->db->where('id',$q4->id)->update('db_delivery', $delivery_entry);
						$delivery_id = $q4->id;
		...
		for($i=1;$i<=$rowcount;$i++){
			...
			if(isset($_REQUEST['tr_item_id_'.$i]) && !empty($_REQUEST['tr_item_id_'.$i])){
				...
				$delivery_qty			=$this->xss_html_filter(trim($_REQUEST['td_data_'.$i.'_3']));
				...
				//$discount_amt 		=($delivery_qty * $unit_total_cost)*$discount_input/100;
				...
				$discount_amt_per_unit = $discount_amt/$delivery_qty;
				...
				$deliveryitems_entry = array(
		    				...
		    				'status'	 		=> 1,
		    				'seller_points'		=> get_seller_points($item_id) * $delivery_qty,

		    			);
				
				$deliveryitems_entry['store_id']=(store_module() && is_admin()) ? $store_id : get_current_store_id();  	
				$q2 = $this->db->insert('db_deliveryitems', $deliveryitems_entry);
		...
		if(isset($send_sms) && $customer_id!=1){
			if(send_sms_using_template($delivery_id,1)==true){
		...
		$this->session->set_flashdata('success', 'Success!! Record Saved Successfully!'.get_init_code('delivery'));
	...
	public function get_details($id,$data){
		//Validate This delivery already exist or not
		$query=$this->db->query("select * from db_delivery where upper(id)=upper('$id')");
		if($query->num_rows()==0){
			show_404();exit;
		}
		else{
			...
			$data['delivery_price']=$query->delivery_price;
			$data['delivery_price']=$query->delivery_price;
	...
	public function delete_delivery($ids){
		...
		$converted_rec = $this->db->select("*")->where("delivery_id in($ids)")->get("db_sales");
		if($converted_rec->num_rows()>0){
			$i=1;
			echo "Can't Delete!<br>These deliverys List Have the Sales Records!";
	...
	public function get_items_info($rowcount,$item_id){
		...
		//Get Customer Price 
		$delivery_price = get_price_level_price($customer_id,$res1->sales_price);
		...
		$item_tax_amt = ($res1->tax_type=='Inclusive') ? calculate_inclusive($delivery_price,$q3->tax) :calculate_exclusive($delivery_price,$q3->tax);
		...
		$info = array(
							...
							'item_delivery_price' 			=> $delivery_price, 
	...
	public function return_delivery_list($sales_id){
		...
		foreach ($q1->result() as $res1) {
			...
			$info = array(
							...
							'item_delivery_price' 			=> $res1->price_per_unit, 
							'item_tax_name' 			=> $q3->tax_name, 
							'item_delivery_qty' 			=> $res1->sales_qty, 
	...
	public function return_row_with_data($rowcount,$info){
		extract($info);
		$item_amount = ($item_delivery_price * $item_delivery_qty) + $item_tax_amt;
		?>
            <tr id="row_<?=$rowcount;?>" data-row='<?=$rowcount;?>'>
               <td id="td_<?=$rowcount;?>_1">
                  <label class='form-control' style='height:auto;' data-toggle="tooltip" title='Edit ?' >
                  <a id="td_data_<?=$rowcount;?>_1" href="javascript:void()" onclick="show_delivery_item_modal(<?=$rowcount;?>)" title=""><?=$item_name;?></a> 
                  		<i onclick="show_delivery_item_modal(<?=$rowcount;?>)" ></i>
                  	</label>
               </td>
               ...
               <td id="td_<?=$rowcount;?>_3">
                  <div class="input-group ">
                     ...
                     <input type="text" style="width:100%" value="<?=format_qty($item_delivery_qty);?>" class="form-control no-padding text-center" readonly onkeyup="calculate_tax(<?=$rowcount;?>)" id="td_data_<?=$rowcount;?>_3" name="td_data_<?=$rowcount;?>_3">
               ...
               <td id="td_<?=$rowcount;?>_10" class="hidden"><input type="text" name="td_data_<?=$rowcount;?>_10" id="td_data_<?=$rowcount;?>_10" class="form-control  text-right no-padding only_currency text-center" onkeyup="calculate_tax(<?=$rowcount;?>)" value="<?=store_number_format($item_delivery_price,0);?>"></td>

               <!-- Discount -->
               <td id="td_<?=$rowcount;?>_8" class="hidden">
                  <input type="text" data-toggle="tooltip" title="Click to Change" name="td_data_<?=$rowcount;?>_8" id="td_data_<?=$rowcount;?>_8" class="pointer  form-control text-right no-padding only_currency text-center item_discount" value="<?=store_number_format($item_discount,0);?>" onclick="show_delivery_item_modal(<?=$rowcount;?>)" readonly>
               </td>

               <!-- Tax Amount -->
               <td id="td_<?=$rowcount;?>_11" class="hidden">
               ...
               <!-- Tax Details -->
               <td id="td_<?=$rowcount;?>_12" class="hidden">
                  <label class='form-control ' style='width:100%;padding-left:0px;padding-right:0px;'>
                  <a id="td_data_<?=$rowcount;?>_12" href="javascript:void()" data-toggle="tooltip" title='Click to Change' onclick="show_delivery_item_modal(<?=$rowcount;?>)" title=""><?=$item_tax_name ;?></a>
                  	</label>
               </td>

               <!-- Amount -->
               <td id="td_<?=$rowcount;?>_9" class="hidden"><input type="text" name="td_data_<?=$rowcount;?>_9" id="td_data_<?=$rowcount;?>_9" class="form-control text-right no-padding only_currency text-center" style="border-color: #f39c12;" readonly value="<?=store_number_format($item_amount,2);?>"></td>
               
               <!-- ADD button -->
               <td id="td_<?=$rowcount;?>_16" style="text-align: center;" class="hidden">
               ...
               <input type="hidden" id="td_data_<?=$rowcount;?>_4" name="td_data_<?=$rowcount;?>_4" value="<?=$item_delivery_price;?>">
