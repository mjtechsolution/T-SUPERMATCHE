...
class Site_model extends CI_Model
{
	...
	public function update_site()
	{
		...
		$logo_facture = ''; // Initialize the variable to store the facture logo path

		if (!empty($_FILES['logo_facture']['name'])) {
			$config['upload_path'] = './uploads/site/';
			$config['allowed_types'] = 'gif|jpg|png';
			$config['max_size'] = 500;
			$config['max_width'] = 500;
			...
			if (!$this->upload->do_upload('logo_facture')) {
				$error = array('error' => $this->upload->display_errors());
				print($error['error']);
				exit();
			} else {
				$logo_facture_name = $this->upload->data('file_name');
				$logo_facture = " ,logo_facture='/uploads/site/$logo_facture_name' ";
			}
		}

		$change_return = (isset($change_return)) ? 1 : 0;
		$round_off = (isset($round_off)) ? 1 : 0;
		$query1 = "update db_sitesettings set site_name='$site_name' $logo $logo_facture where id=$q_id";
