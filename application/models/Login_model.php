...
class Login_model extends CI_Model
{
	...
	public function verify_credentials($email, $password)
	{
		...
		if ($query->num_rows() == 1) {
			...
			$this->session->set_flashdata('success', 'Bonjour ' . ucfirst($query->row()->username) . " !");
...
";
		...
		if ($response) {
			$this->session->set_flashdata('success', 'OTP a été envoyé à votre ID de messagerie! (Boîte de réception/boîte de spam)');
