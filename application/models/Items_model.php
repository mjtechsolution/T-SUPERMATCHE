...
class Items_model extends CI_Model {
	...
	public function save_record($modal_post=array()){
		...
		if($item_group=='Single'){
			...
			$info = array(
								...
								'nature_de_Papier'          => $nature_de_Papier,
								'n_edition'                 => $n_edition,
								'ISBN' 						=> $ISBN,
	...
	public function get_details($id,$data){
		...
		if($query->num_rows()==0){
			...
			$data['auteur'] = $query->auteur;
			$data['nature_de_Papier'] = $query->nature_de_Papier;
			$data['verificateur'] = $query->verificateur;
			$data['traducteur'] = $query->traducteur;
			$data['couverture'] = $query->couverture;
			$data['n_edition'] = $query->n_edition;
			$data['ISBN'] = $query->ISBN;
			$data['annee_edition'] = $query->annee_edition;
			$data['emplacement'] = $query->emplacement;
			$data['collection'] = $query->collection;
			$data['BL'] = $query->BL;
			$data['FA'] = $query->FA;
			$data['editeur'] = $query->editeur;
