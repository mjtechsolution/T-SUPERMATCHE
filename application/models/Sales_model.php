...
?>
			...
			<div class="modal-dialog ">
				<div class="modal-content">
					...
					<div class="modal-footer">
						...
						<button type="button" class="btn btn-default btn-lg" data-dismiss="modal"><?= $this->lang->line('close') ?></button>
	...
	public function view_livrison_modal($sales_id)
	{
		$q1 = $this->db->query("select * from db_sales where id=$sales_id");
		$res1 = $q1->row();
		$customer_id = $res1->customer_id;
		$q2 = $this->db->query("select * from db_customers where id=$customer_id");
		$res2 = $q2->row();
		$q3 = $this->db->query("select * from bd_delivery where sales_id = '$sales_id'");
		$res3 = $q3->row();

		$address = $res3->address;
		$note = $res3->note;
		$delivered_by 		= $res3->delivered_by;
		$received_by 		= $res3->received_by;
		...
		$sales_date = ($res3->delivery_date ?  $res3->delivery_date  : $res1->sales_date);
		$reference_no = $res1->reference_no;
		$sales_code = $res1->sales_code;
		$sales_note = $res1->sales_note;
		$grand_total = $res1->grand_total;
		...
		$reference_no = $res1->reference_no;
	?>
		<form class="form-horizontal" id="delivery-form" onkeypress="return event.keyCode != 13;">
			<input type="hidden" id="base_url" value="<?php echo $base_url;; ?>">
			<input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
			<div class="modal fade" id="view_livrison_modal" tabindex='-1'>
				<div class="modal-dialog modal-lg">
					<div class="modal-content">
						<div class="modal-header header-custom">
							<button type="button" class="close" data-dismiss="modal" aria-label="Close">
								<span aria-hidden="true">&times;</span></button>
							<h4 class="modal-title text-center">AJOUTER UNE LIVRAISON</h4>
						</div>
						<div class="modal-body">

							<div class="livrison_box">
									...
									<div class="livrison_body">
										...
										<div class="flex">
											<div class="group_input">
												...
												<input type="data" value="<?= $sales_date  ?>" id="sales_date" name="sales_date" onkeyup="shift_cursor(event,'status')">
											...
											<div class="group_input">
												...
												<select name="" id="status" name="status" onkeyup="shift_cursor(event,'reference_no_delivery')">
													<option value="Enbalage">Enbalage</option>
										...
										<div class="flex">
											<div class="group_input">
												...
												<input type="text" name="reference_no_delivery" id="reference_no_delivery" onkeyup="shift_cursor(event,'delivered_by')" value="<?= 'BM_' . $reference_no . '/' . $sales_id ?>">
												<span id="reference_no_delivery_msg" style="display:none" class="text-danger"></span>
											...
											<div class="group_input">
												...
												<input type="text" name="delivered_by" id="delivered_by" onkeyup="shift_cursor(event,'reference_no')" value="<?= $delivered_by ?>">
											</div>
										</div>
										<div class="flex">
											<div class="group_input">
												...
												<input type="text" value="<?= $reference_no ?>" name="reference_no" id="reference_no" onkeyup="shift_cursor(event,'received_by')">
											...
											<div class="group_input">
												...
												<input type="text" name="received_by" id="received_by" onkeyup="shift_cursor(event,'customer_name')" value="<?= $received_by ?>">
											</div>
										</div>
										<div class="flex">
											<div class="group_input">
												...
												<input type="text" value="<?= $customer_name ?>" id="customer_name" name="customer_name" onkeyup="shift_cursor(event,'attachement')">
											...
											<div class="group_input">
												...
												<input type="text" id="attachement" name="attachement" onkeyup="shift_cursor(event,'addresse')">
										...
										<div class="flex">
											<div class="group_input">
												...
												<textarea name="addresse" id="addresse" cols="30" rows="5" onkeyup="shift_cursor(event,'note')"><?= $address ?>
											</textarea>
											</div>
											<div class="group_input">
												<label>
												...
												<textarea name="note" id="note" cols="30" rows="5" onkeyup="shift_cursor(event,'note')">
											<?= $note ?></textarea>
												<span id="addresse_msg" style="display:none" class="text-danger"></span>
						...
						<div class="modal-footer">
							...
							<button type="button" id="save" name="save" class="btn btn-primary btn-lg">Enregistrer</button>
		...
		<script type="text/javascript">
			$("#save,#update").on("click", function() {
				...
				var reference_no_delivery = document.getElementById("reference_no_delivery").value;
				...
				check_field("status");
				check_field("reference_no_delivery");
				check_field("reference_no");
				...
				$.post(
					base_url + "sales/sales_delivery_save_and_update", {
						command: <?= $sales_id ?>,
						sales_date: sales_date,
						status_delivery: status,
						reference_no_delivery: reference_no_delivery,
...
<?php
	...
	public function sales_delivery_verify_save_and_update()
	{


		//check if it save or update




		$this->db->query("ALTER TABLE bd_delivery AUTO_INCREMENT = 1");


		$a = $this->db->query("select count(*) from bd_delivery where sales_id = '$command'");
		...
		$count_id = get_last_count_id('bd_delivery');
		if ($count_id) {
			$count_id = $count_id;
		}

		//genrate delivery code 
		$formatted_store_id = sprintf('%02d', $warehouse_id);

		// Format item ID to four digits
		$formatted_item_id = sprintf('%04d', $count_id);

		// Combine the formatted values with the constant string "IC"
		$delivery_code = 'DC' . $formatted_store_id . $formatted_item_id;
		...
		if ($a > 0) {
			$array_delivery = array(
				...
				'created_by' 				=> $CUR_USERNAME,
				'system_ip' 				=> $SYSTEM_IP,
				'system_name' 				=> $SYSTEM_IP,
				'Attachement' 				=> $Attachement,
				'status_delivery' 				=> $status_delivery,
				'address' 				=> $address,
				'sales_id' 				=> $command,
			);
			$this->db->where('sales_id', $command);
			if ($this->db->update('bd_delivery', $array_delivery)) {
				$this->session->set_flashdata('success', 'Success!! New Delivery Created Succssfully!!');
				return "success";
			} else {
			...
			$array_delivery = array(
				'store_id' 				=> $store_id,
				'warehouse_id' 					=> 2,
				'count_id' 				=>  $count_id, //get_init_code('sales'),
				/*Coupon disocunt amt*/
				'delivery_code' 				=> $delivery_code,
				'reference_no' 				=> $reference_no,
				'reference_no_delivery' 				=> $reference_no_delivery,
			...
			if ($this->db->insert('bd_delivery', $array_delivery)) {
				$this->session->set_flashdata('success', 'Success!! New Delivery Created Succssfully!!');
				return "success";
			} else {
