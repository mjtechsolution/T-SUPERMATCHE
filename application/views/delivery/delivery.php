...
<body class="hold-transition skin-blue sidebar-mini">
   <div class="wrapper">
      ...
      $CI = &get_instance();
      if (!isset($sales_id)) {
         $customer_id  = $delivery_date = $delivery_status = $warehouse_id =
            $reference_no  =
            $other_charges_input          = $other_charges_tax_id = $store_id =
            $discount_type  = $delivery_note = '';
         $delivery_date = show_date(date("d-m-Y"));
         $expire_date = '';
         $discount_input = 0;
         ...
         $delivery_date = show_date($q2->row()->delivery_date);
         $expire_date = (!empty($q2->row()->expire_date)) ? show_date($q2->row()->expire_date) : '';
         $delivery_status = $q2->row()->delivery_status;
         ...
         $delivery_note = $q2->row()->delivery_note;
      ...
      <div class="content-wrapper">
         ...
         <section class="content-header">
            <h1>
               <?= $page_title; ?>
               <small><?= $this->lang->line('add/edit') ?> Devis</small>
            </h1>
            <ol class="breadcrumb">
               <li><a href="<?php echo $base_url; ?>dashboard"><i class="fa fa-dashboard"></i> Acceuil</a></li>
               <li><a href="<?php echo $base_url; ?>delivery"><?= $this->lang->line('delivery_list'); ?></a></li>
               <li><a href="<?php echo $base_url; ?>delivery/add">New delivery</a></li>
            ...
            <div class="row">
               ...
               <div class="col-md-12">
                  ...
                  <div class="box box-primary ">
                     ...
                     <!-- form start -->
                     <!-- OK START -->
                     <?= form_open('#', array('class' => 'form-horizontal', 'id' => 'delivery-form', 'enctype' => 'multipart/form-data', 'method' => 'POST')); ?>
                     ...
                     <div class="box-body">
                       ...
                       <div class="form-group">
                           ...
                           <?php
                              if(isset($sales_id)){
                                 echo '<label for="sales_id" class="col-sm-2 control-label">Num vente <label class="text-danger"></label></label>';
                                 echo '<div class="col-sm-3">';
                                 echo '<input type="text" class="form-control" readonly id="sales_id" name="sales_id"    value="'.$sales_id.'">';
                                 echo '</div>';
                              }
                           ?>
                        </div>
                        ...
                        <div class="form-group">
                           ...
                           <label for="delivery_date" class="col-sm-2 control-label"><?= $this->lang->line('delivery_date'); ?> <label class="text-danger">*</label></label>
                           <div class="col-sm-3">
                              <div class="input-group date">
                                 <div class="input-group-addon">
                                    <i class="fa fa-calendar"></i>
                                 </div>
                                 <input type="text" class="form-control pull-right datepicker" id="delivery_date" name="delivery_date" readonly onkeyup="shift_cursor(event,'delivery_status')" value="<?= $delivery_date; ?>">
                              </div>
                              <span id="delivery_date_msg" style="display:none" class="text-danger"></span>
                           </div>
                        </div>
                        <div class="form-group hide">
                           <label for="delivery_status" class="col-sm-2 control-label"><?= $this->lang->line('status'); ?> <label class="text-danger">*</label></label>
                           <div class="col-sm-3">
                              <select class="form-control select2" id="delivery_status" name="delivery_status" style="width: 100%;" onkeyup="shift_cursor(event,'mobile')">
                                 <?php
                                 $pending_select = ($delivery_status == 'delivery') ? 'selected' : '';
                                 ?>
                                 <option <?= $pending_select; ?> value="delivery">delivery</option>
                              </select>
                              <span id="delivery_status_msg" style="display:none" class="text-danger"></span>
                           </div>
                        </div>
                        <div class="form-group ">
                     ...
                     <div class="row">
                                 ...
                                 <div class="box-info">
                                    ...
                                    <div class="box-body">
                                       <div class="table-responsive" style="width: 100%">
                                          <table class="table table-hover table-bordered" style="width:100%" id="delivery_table">
                        ...
                        <div class="col-md-6 hidden">
                           ...
                           <div class="row">
                              <div class="col-md-12">
                                 <div class="form-group">
                                    <label for="delivery_note" class="col-sm-4 control-label"><?= $this->lang->line('note'); ?></label>
                                    <div class="col-sm-8">
                                       <textarea class="form-control text-left" id='delivery_note' name="delivery_note"><?= $delivery_note; ?></textarea>
                                       <span id="delivery_note_msg" style="display:none" class="text-danger"></span>
                                    </div>
                                 </div>
                              </div>
                           </div>
                              ...
                              <div class="box-body ">
                                 <div class="col-md-12">
                                    <div class="checkbox icheck">
                                       <label>
                                          <input type="checkbox" <?= $send_sms_checkbox; ?> class="form-control" id="send_sms" name="send_sms"> <label for="delivery_discount" class=" control-label"><?= $this->lang->line('send_sms_to_customer'); ?>
   ...
   <script src="<?php echo $theme_link; ?>js/delivery/delivery.js"></script>
   ...
   <script>
      ...
      $("#store_id").on("change", function() {
         ...
         $.post(base_url + "delivery/get_customers_select_list", {
            store_id: store_id
         }, function(result) {
         ...
         $.post(base_url + "delivery/get_tax_select_list", {
            store_id: store_id
         }, function(result) {
      ...
      function calculate_tax(i) { //i=Row
         ...
         var delivery_price = parseFloat($("#td_data_" + i + "_10").val());
         $("#td_data_" + i + "_4").val(delivery_price);
         /*Discounr*/
         var discount_amt = $("#td_data_" + i + "_8").val();
         discount_amt = (isNaN(parseFloat(discount_amt))) ? 0 : parseFloat(discount_amt);

         var amt = parseFloat(qty) * delivery_price; //Taxable
      ...
      function show_delivery_item_modal(row_id) {
      ...
      function set_tax_value(row_id) {
         //get the delivery price of the item
         var tax_type = $("#tr_tax_type_" + row_id).val();
         var tax = $("#tr_tax_value_" + row_id).val(); //%
         var qty = $("#td_data_" + row_id + "_3").val();
         qty = (isNaN(qty)) ? 0 : qty;
         var delivery_price = parseFloat($("#td_data_" + row_id + "_10").val());
         delivery_price = (isNaN(delivery_price)) ? 0 : delivery_price;
         delivery_price = delivery_price * qty;
         ...
         //Calculate discount      
         var discount_amt = (item_discount_type == 'Percentage') ? ((delivery_price) * item_discount_input) / 100 : (item_discount_input * qty);
         delivery_price -= parseFloat(discount_amt);

         var tax_amount = (tax_type == 'Inclusive') ? calculate_inclusive(delivery_price, tax) : calculate_exclusive(delivery_price, tax);
   ...
   <script type="text/javascript">
      <?php if (isset($sales_id)) { ?>
         ...
         $(document).ready(function() {
            ...
            $.post(base_url + "delivery/return_delivery_list/" + sales_id, {}, function(result) {
               //alert(result);
