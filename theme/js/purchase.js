...
$("#save,#update").on("click", function (e) {
  ...
  if (flag == false) {
    toastr["error"]("Je m'excuse si j'ai manqué quelque chose à remplir");
    return;
  }
  ...
  if (flag1 == false) {
    toastr["warning"]("Veuillez sélectionner l’article!! ");
    $("#item_search").focus();
    return;
  }
  //end
  ...
  $.ajax({
    ...
    url:
      base_url +
      "purchase/purchase_save_and_update?command=" +
      this_id +
      "&rowcount=" +
      rowcount +
      "&tot_subtotal_amt=" +
...
$("#item_search").autocomplete({
  source: function (data, cb) {
    $.ajax({
      ...
      beforeSend: function () {
        if ($("#warehouse_id").val() == "") {
          toastr["warning"]("Veuillez sélectionner Wareshouse!");
...
function return_row_with_data(item_id) {
  ...
  var rowcount = $("#hidden_rowcount").val();
  $.post(
    base_url + "purchase/return_row_with_data/" + rowcount + "/" + item_id,
    {},
    function (result) {
      //alert(result);
...
function calculate_sales_price(rowcount) {
  var purchase_price = isNaN(
    parseFloat($("#td_data_" + rowcount + "_10").val())
  )
    ? 0
    : parseFloat($("#td_data_" + rowcount + "_10").val());
  var profit_margin = isNaN(parseFloat($("#td_data_" + rowcount + "_12").val()))
    ? 0
    : parseFloat($("#td_data_" + rowcount + "_12").val());
  var tax_type = $("#tax_type").val();
  var sales_price = parseFloat(0);
  sales_price =
    purchase_price + (purchase_price * profit_margin) / parseFloat(100);
...
function calculate_profit_margin(rowcount) {
  var purchase_price = isNaN(
    ...
    ? 0
    : parseFloat($("#td_data_" + rowcount + "_10").val());
  var sales_price = isNaN(parseFloat($("#td_data_" + rowcount + "_13").val()))
    ? 0
    : parseFloat($("#td_data_" + rowcount + "_13").val());
...
function delete_payment(payment_id) {
  if (confirm("Do You Wants to Delete Record ?")) {
    ...
    $.post(
      base_url + "purchase/delete_payment",
...
function delete_purchase(q_id) {
  ...
  if (confirm("Do You Wants to Delete Record ?")) {
    ...
    $.post(
      base_url + "purchase/delete_purchase",
      { q_id: q_id },
      function (result) {
        //alert(result);return;
        if (result == "success") {
...
function pay_now(purchase_id) {
  var base_url = $("#base_url").val();
  $.post(
    base_url + "purchase/show_pay_now_modal",
...
function view_payments(purchase_id) {
  var base_url = $("#base_url").val();
  $.post(
    base_url + "purchase/view_payments_modal",
    { purchase_id: purchase_id },
    function (result) {
...
function save_payment(purchase_id) {
  ...
  if (amount == 0) {
    toastr["error"]("Veuillez entrer un montant valide!");
    return false;
  }

  if (amount > parseFloat($("#due_amount_temp").html())) {
    toastr["error"](
      "Le montant saisi ne doit pas être supérieur au montant dû!"
    );
    return false;
  }
  ...
  $.post(
    base_url + "purchase/save_payment",
    {
