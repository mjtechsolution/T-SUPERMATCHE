...
$("#save,#update").on("click", function (e) {
  ...
  if (get_float_type_data(amount) <= 0) {
    toastr["warning"]("Le montant doit être supérieur à zéro !!");
    return;
  }

  if ($("#debit_account_id").val() == $("#credit_account_id").val()) {
    toastr["warning"]("Les deux comptes ne devraient pas être les mêmes !!");
    return;
  }
...
function delete_money_transfer(q_id) {
  ...
  if (confirm("Do You Wants to Delete Record ?")) {
    ...
    $.post(
      base_url + "money_transfer/delete_money_transfer",
      { q_id: q_id },
      function (result) {
        //alert(result);return;
        result = result.trim();
