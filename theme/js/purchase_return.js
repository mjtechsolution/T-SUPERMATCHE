...
$("#save,#update,#create").on("click", function (e) {
  ...
  $.ajax({
    ...
    url:
      base_url +
      "purchase_return/purchase_save_and_update?command=" +
      this_id +
      "&rowcount=" +
      rowcount +
      "&tot_subtotal_amt=" +
...
$("#item_search").autocomplete({
      ...
      success: function (res) {
        ...
        if (res.length) {
          result = $.map(res, function (el) {
            return {
              label:
                el.item_code + "--" + el.label + " [STOCK:" + el.stock + "]",
              value: "",
              id: el.id,
              item_name: el.value,
              stock: el.stock,
  ...
  select: function (e, ui) {
    ...
    if (parseFloat(stock) == 0) {
      toastr["warning"]("Seulement 0 articles en stock!!");
      return false;
    }
...
function return_row_with_data(item_id) {
  ...
  $.post(
    base_url +
      "purchase_return/return_row_with_data/" +
      rowcount +
      "/" +
      item_id,
    ...
    function (result) {
      ...
      if (result == "item_not_exist") {
        toastr["error"](
          "Pardon! Cet objet n’existe pas dans cette entrée d’achat!!"
        );
      } else {
...
function delete_payment(payment_id) {
  if (confirm("Do You Wants to Delete Record ?")) {
    ...
    $.post(
      base_url + "purchase_return/delete_payment",
...
function delete_return(q_id) {
  ...
  if (confirm("Do You Wants to Delete Record ?")) {
    ...
    $.post(
      base_url + "purchase_return/delete_return",
      { q_id: q_id },
      function (result) {
        //alert(result);return;
        if (result == "success") {
...
function pay_now(purchase_id) {
  var base_url = $("#base_url").val();
  $.post(
    base_url + "purchase_return/show_pay_now_modal",
    { purchase_id: purchase_id },
    function (result) {
...
function view_payments(purchase_id) {
  var base_url = $("#base_url").val();
  $.post(
    base_url + "purchase_return/view_payments_modal",
    { purchase_id: purchase_id },
    function (result) {
...
function save_payment(return_id) {
  ...
  $.post(
    base_url + "purchase_return/save_payment",
    {
