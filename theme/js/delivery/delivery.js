...
$("#save,#update").on("click", function (e) {
  ...
  //Atleast one record must be added in delivery table
  ...
  $.ajax({
    ...
    url:
      base_url +
      "delivery/delivery_save_and_update?command=" +
      this_id +
      "&rowcount=" +
      rowcount +
      "&tot_subtotal_amt=" +
    ...
    success: function (result) {
      ...
      result = result.split("<<<###>>>");
      if (result[0] == "success") {
        location.href = base_url + "delivery/invoice/" + result[1];
...
$("#item_search").autocomplete({
  source: function (data, cb) {
    if ($("#customer_id").val() == null) {
      toastr["error"]("Veuillez sélectionner le client");
...
function return_row_with_data(item_id) {
  ...
  $.post(
    base_url + "delivery/return_row_with_data/" + rowcount + "/" + item_id,
...
function delete_delivery(q_id) {
  ...
  if (confirm("Do You Wants to Delete Record ?")) {
    ...
    $.post(
      base_url + "delivery/delete_delivery",
      { q_id: q_id },
      function (result) {
        //alert(result);return;
        if (result == "success") {
...
function multi_delete() {
  ...
  if (confirm("Are you sure ?")) {
    ...
    $.ajax({
      type: "POST",
      url: base_url + "delivery/multi_delete",
      data: data,
      cache: false,
      contentType: false,
      processData: false,
