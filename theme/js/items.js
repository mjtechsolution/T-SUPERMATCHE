$("#save,#update").on("click", function (e) {
  ...
  if (item_group == "Single") {
    ...
    if (!validate_variants_records()) {
      //toastr["warning"]("Je m'excuse si j'ai manqué quelque chose à remplir");
      return;
    }
  }
  ...
  if (item_group == "Variants") {
    ...
    if (existing_row_count == 1) {
      toastr["warning"]("Aucun enregistrement dans la liste des variantes!");
      return;
    }
  }
  ...
  if (this_id == "save") {
    ...
    $.ajax({
      ...
      url:
        base_url +
        "items/update_items?existing_row_count=" +
        existing_row_count,
      data: data,
      cache: false,
      contentType: false,
...
function delete_items(q_id) {
  if (confirm("Do You Wants to Delete Record ?")) {
    ...
    $.post(
      $("#base_url").val() + "items/delete_items",
      { q_id: q_id },
      function (result) {
        //alert(result);return;
        if (result == "success") {
...
function calculate_purchase_price() {
  ...
  var tax = isNaN(parseFloat($("option:selected", "#tax_id").attr("data-tax")))
    ? 0
    : parseFloat($("option:selected", "#tax_id").attr("data-tax"));
  tax = parseFloat(tax);
...
function calculate_sales_price() {
  var price = isNaN(parseFloat($("#price").val()))
    ? 0
    : parseFloat($("#price").val());
  var profit_margin = isNaN(parseFloat($("#profit_margin").val()))
    ? 0
    : parseFloat($("#profit_margin").val());
...
function calculate_profit_margin() {
  var price = isNaN(parseFloat($("#price").val()))
    ? 0
    : parseFloat($("#price").val());
  var sales_price = isNaN(parseFloat($("#sales_price").val()))
    ? 0
    : parseFloat($("#sales_price").val());
...
function view_warehouse_wise_stock_item(item_id) {
  var base_url = $("#base_url").val();
  $.post(
    base_url + "warehouse/view_warehouse_wise_stock_item",
    { item_id: item_id },
    function (result) {
...
$("#variant_search").autocomplete({
  source: function (data, cb) {
    $.ajax({
      ...
      beforeSend: function () {
        if ($("#tax_id").val() == "") {
          toastr["warning"]("Veuillez sélectionner le type de taxe!");
          $("#tax_id").select2("open");
...
function return_variant_data_in_row(variant_id) {
  ...
  $.post(
    base_url +
      "items/return_variant_data_in_row/" +
      rowcount +
      "/" +
      variant_id,
    {},
...
function calculate_purchase_price_new(price) {
  ...
  purchase_price =
    tax_type == "Inclusive"
      ? price
      : parseFloat(price) + parseFloat(calculate_exclusive(price, tax));
...
$("#item_group").on("change", function (event) {
  ...
  if (item_group == "Variants") {
    $(
      "#price,#purchase_price,#profit_margin,#sales_price,#mrp,#hsn,#sku,#custom_barcode,#adjustment_qty"
    )
      .parent()
      .addClass("hide");
    $(".variant_div").show();
  ...
  for (i = 1; i <= rowcount; i++) {
    if (document.getElementById("td_data_" + i + "_3")) {
      ...
      if (purchase_price == 0 || sales_price == 0) {
        $("#td_data_" + i + "_3").focus();
        toastr["warning"]("Prix des variantes et prix de vente est requis!");
        return false;
      }
    } //if end
  } //for end
