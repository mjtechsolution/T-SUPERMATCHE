...
$("#save,#update").on("click", function (e) {
  ...
  $.ajax({
    ...
    url:
      base_url +
      "stock_transfer/stock_save_and_update?command=" +
      this_id +
      "&rowcount=" +
      rowcount,
    data: data,
...
$("#item_search").autocomplete({
    ...
    $.ajax({
      ...
      beforeSend: function () {
        if ($("#warehouse_from").val() == "") {
          toastr["warning"]("Veuillez choisir parmi Wareshouse!");
        ...
        if ($("#warehouse_from").val() == $("#warehouse_to").val()) {
          toastr["warning"](
            "Destination Wareshouse ne devrait pas être la même!"
          );
          $("#warehouse_to").select2("open");
          $("#item_search").removeClass("ui-autocomplete-loading");
          return;
  ...
  select: function (e, u) {
    ...
    if (parseFloat(u.item.stock) <= 0) {
      toastr["warning"](u.item.stock + " Articles en stock!!");
...
function return_row_with_data(item_id) {
  ...
  $.post(
    base_url +
      "stock_transfer/return_row_with_data/" +
      rowcount +
      "/" +
      item_id,
...
function delete_stock(q_id) {
  if (confirm("Do You Wants to Delete Record ?")) {
    ...
    $.post(
      base_url + "stock_transfer/delete_stock",
      { q_id: q_id },
      function (result) {
        //alert(result);return;
        if (result == "success") {
