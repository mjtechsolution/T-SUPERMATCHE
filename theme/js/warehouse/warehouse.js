...
$("#save,#update").on("click", function () {
  ...
  if (email != "" && !validateEmail(email)) {
    $("#email_msg").html("E-mail non valide!").show();
    toastr["warning"]("E-mail non valide!");
    return false;
  }
  ...
  $.post(
    base_url + "warehouse/save_or_update",
    {
      command: this_id,
      q_id: q_id,
    ...
    function (result) {
      ...
      if (result == "success") {
        ...
        toastr["error"](
          "Pardon! Impossible de mettre à jour Record.Essayez à nouveau!"
        );
      } else {
        toastr["error"](result);
      }
...
function update_status(id, status) {
  var base_url = $("#base_url").val();
  $.post(
    base_url + "warehouse/status_update",
    { id: id, status: status },
    function (result) {
      //alert(result);return;
      if (result == "success") {
        ...
        toastr["error"]("Erreur! Quelque chose s’est mal passé!");
        failed.currentTime = 0;
        failed.play();
      }
      return false;
