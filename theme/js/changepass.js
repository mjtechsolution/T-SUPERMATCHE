$("#save").on("click", function () {
  ...
  if (currentpass == "") {
    toastr["warning"]("Entrez le mot de passe actuel!");
    document.getElementById("current_pass").focus();
    return;
  }
  if (newpass == "") {
    toastr["warning"]("Veuillez entrer un nouveau mot de passe!");
    $("#pass").focus();
    return;
  }
  if (retypepass == "") {
    toastr["warning"]("S’il vous plaît confirmer le mot de passe!");
    $("#confirm").focus();
    return;
  }
  if (newpass != retypepass) {
    ...
    if (confirm("Are you Sure ?")) {
      /*Check XSS Code*/
      if (!xss_validation(currentpass)) {
        return false;
      }
      if (!xss_validation(newpass)) {
        return false;
      }

      //Send data form to php
      $.post(
        base_url + "users/password_update",
