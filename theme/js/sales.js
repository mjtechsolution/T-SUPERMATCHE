...
$("#save,#update").on("click", function (e) {
  ...
  if (
    ...
    if (parseFloat($("#total_amt").text()) != parseFloat($("#amount").val())) {
      $("#amount").focus();
      toastr["warning"](
        "Le client sans rendez-vous devrait payer le montant total !!"
      );
      return;
    }

    if ($("#payment_type").val() == "") {
      toastr["warning"]("Veuillez sélectionner le type de paiement!!");
      return;
    }
  }
  if (this_id == "save") {
...
function return_row_with_data(item_id) {
  ...
  $.post(
    base_url + "sales/return_row_with_data/" + rowcount + "/" + item_id,
...
function restore_customer_list() {
  ...
  $.post(
    base_url + "customers/restore_customer_list",
...
function pay_now(sales_id) {
  var base_url = $("#base_url").val();
  $.post(
    base_url + "sales/show_pay_now_modal",
    { sales_id: sales_id },
    function (result) {
...
function view_payments(sales_id) {
  var base_url = $("#base_url").val();
  $.post(
    base_url + "sales/view_payments_modal",
    { sales_id: sales_id },
    function (result) {
...
function save_payment(sales_id) {
  ...
  $.post(
    base_url + "sales/save_payment",
    {
      cheque_number: cheque_number,
      cheque_period: cheque_period,
      allow_tot_advance: allow_tot_advance,
      customer_id: customer_id,
...
function restrict_quantity(item_id, service_bit = 0) {
  ...
  if (
    service_bit == 0 &&
    available_qty != 0 &&
    count_item_qty >= available_qty
  ) {
    toastr["warning"]("Seulement " + available_qty + " Articles en stock!!");
    failed.currentTime = 0;
    failed.play();
    return false;
  }
...
function get_coupon_details() {
  ...
  if (coupon_code != "") {
    ...
    $.post(
      base_url + "customer_coupon/get_coupon_details",
      {
        invoice_type: "sales",
