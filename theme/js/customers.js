...
$("#save,#update").on("click", function (e) {
  ...
  if (email != "" && !validateEmail(email)) {
    ...
    //flag=false;
    toastr["warning"]("Veuillez saisir un identifiant de messagerie valide.");
    return;
  } else {
    $("#email_msg").html("E-mail non valide!").hide();
  }
...
function update_status(id, status) {
  var base_url = $("#base_url").val();
  $.post(
    base_url + "customers/update_status",
    { id: id, status: status },
    function (result) {
      if (result == "success") {
        toastr["success"]("Enregistrement mis à jour avec succès!");
...
function delete_customers(q_id) {
  ...
  if (confirm("Do You Wants to Delete Record ?")) {
    ...
    $.post(
      base_url + "customers/delete_customers",
      { q_id: q_id },
      function (result) {
        // alert(result);return;
        if (result == "success") {
...
function pay_now(customer_id) {
  $.post(
    $("#base_url").val() + "customers/show_pay_now_modal",
...
function save_payment(customer_id) {
  ...
  $.post(
    base_url + "customers/save_payment",
    {
...
function pay_return_due(customer_id) {
  $.post(
    $("#base_url").val() + "customers/show_pay_return_due_modal",
    { customer_id: customer_id },
    function (result) {
...
function save_return_due_payment(customer_id) {
  ...
  $(".payment_save").attr("disabled", true); //Enable Save or Update button
  $.post(
    base_url + "customers/save_return_due_payment",
    {
      account_id: account_id,
      customer_id: customer_id,
      payment_type: payment_type,
...
function delete_opening_balance_entry(entry_id) {
  if (confirm("Do You Wants to Delete Record ?")) {
    ...
    $.post(
      base_url + "customers/delete_opening_balance_entry",
      { entry_id: entry_id, customer_id: $("#q_id").val() },
      function (result) {
        //alert(result);//return;
        result = result;
        if (result == "success") {
...
function view_payments(customer_id) {
  $.post(
    $("#base_url").val() + "customers/view_payment_list_modal",
    { customer_id: customer_id },
    function (result) {
      $(".pay_now_modal").html("").html(result);
      //Date picker
