...
function update_status(id, status) {
  var base_url = $("#base_url").val();
  $.post(
    base_url + "gateways/update_status",
    { id: id, status: status },
    function (result) {
      if (result == "success") {
        toastr["success"]("Enregistrement mis à jour avec succès!");
...
function delete_gateways(q_id) {
  ...
  if (confirm("Do You Wants to Delete Record ?")) {
    ...
    $.post(
      base_url + "gateways/delete_gateways",
      { q_id: q_id },
      function (result) {
        // alert(result);return;
        if (result == "success") {
...
function pay_now(gateway_id) {
  $.post(
    $("#base_url").val() + "gateways/show_pay_now_modal",
...
function save_payment(gateway_id) {
  ...
  $.post(
    base_url + "gateways/save_payment",
    {
...
function pay_return_due(gateway_id) {
  $.post(
    $("#base_url").val() + "gateways/show_pay_return_due_modal",
    { gateway_id: gateway_id },
    function (result) {
...
function save_return_due_payment(gateway_id) {
  ...
  $(".payment_save").attr("disabled", true); //Enable Save or Update button
  $.post(
    base_url + "gateways/save_return_due_payment",
    {
      account_id: account_id,
      gateway_id: gateway_id,
      payment_type: payment_type,
...
function delete_opening_balance_entry(entry_id) {
  if (confirm("Do You Wants to Delete Record ?")) {
    ...
    $.post(
      base_url + "gateways/delete_opening_balance_entry",
      { entry_id: entry_id, gateway_id: $("#q_id").val() },
      function (result) {
        //alert(result);//return;
        result = result;
        if (result == "success") {
...
function view_payments(gateway_id) {
  $.post(
    $("#base_url").val() + "gateways/view_payment_list_modal",
    { gateway_id: gateway_id },
    function (result) {
      $(".pay_now_modal").html("").html(result);
      //Date picker
