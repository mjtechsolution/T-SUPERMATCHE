...
//$('.make_sale').on("click",function (e) {
    ...
    if($(".items_table tr").length==1){
    	toastr["warning"]("Liste de ventes vide.");
		return;
    }
   ...
   /* var paid_amt=$(".sales_div_tot_paid").text();
    ...
    if($('option:selected', "#customer_id").attr('data-delete_bit')==1 && balance!=0){
    	toastr["warning"]("Le client qui se présente doit payer le montant total.");
		return;
    }
		...
		$.ajax({
			...
			success: function(result){
				...
				result=result.split("<<<###>>>");
					...
					{
					   toastr['error']("Désolé ! Échec de l'enregistrement. Veuillez réessayer.");
					}
					else
					{
						alert(result);
...
$('#hold_invoice').on("click",function (e) {
	...
	if($(".items_table tr").length==1){
    	toastr["error"]("Veuillez sélectionner des éléments dans la liste !");
	...
	swal({
		title: "Facture en attente ? La même référence remplacera la liste précédente si elle existe !",icon: "warning",buttons: true,dangerMode: true,
		content: {
			element: "input",attributes: 
			{
				placeholder: "Veuillez entrer le numéro de référence !",
				type: "text",
	...
	.catch(err => {
	    toastr['error']("Échec ! La facture n'a pas été enregistrée ! <br/>Veuillez entrer le numéro de référence.");
	    failed.currentTime = 0;
		failed.play();
	});//swal end
...
function hold_invoice_delete(invoice_id){
	swal({ title: "Êtes-vous sûr(e) ?",icon: "warning",buttons: true,dangerMode: true,}).then((sure) => {
	if(sure) {//confirmation start
	var base_url=$("#base_url").val();
  $.post(base_url+"pos/hold_invoice_delete/"+invoice_id,{},function(result){
  	result=result;
    if(result=='success'){
    	toastr["success"]("Succès ! La facture a été supprimée !");
	    success.currentTime = 0;
		success.play();
	    hold_invoice_list();
    }
    else{
    	toastr['error']("Échec de la suppression de la facture ! Veuillez réessayer.");
    	failed.currentTime = 0;
		failed.play();
    }
  });
